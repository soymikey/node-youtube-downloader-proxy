...
function processData (data, options, stream) {
  ...
  const r = request.defaults({ proxy: 'http://127.0.0.1:1080' })
  const req = r({ url: item.url, headers: headers, ecdhCurve: 'auto' })
 ...
 * @param {!Object} options
 */
const ytdl = (module.exports = function (videoUrl, options) {
  ...
  ytdl.getInfo(videoUrl, options, function getInfo (err, data) {
